...
class SyncDb_MySQL {
	...
	private function Initialize(){
		...
		$this->_autoIncrementFields = array();
	}
		...
		foreach($newDbStructure as $tableName=>$newFields){ //for each table
			...
			if ($this->_dbManager->NumRows() == 0) { //table does not exist
				...
				foreach($indexRows as $indexRow){
					...
					if(!empty($indexesFinal[$fieldName])){
						...
						if($thisIndexName !== "PRIMARY"){
							...
							$this->_indexesToDrop[$tableName][$fieldName][] = $thisIndexName;
							$setIndex = false; //dont use this index
						}
						else {
							//this index may be our PRIMARY index, so let's remove the OLD index
							$this->_indexesToDrop[$tableName][$fieldName][] = $oldIndexName;
						}
					}
					...
					if($setIndex){
						...
						$indexesFinal[$fieldName]['IndexPrefixLength'] = $indexRow['Sub_part']; //ie NULL or <int> of index prefix length (size of characters to index, needed for text/blob columns)
					}
				}
				...
				foreach ($result as $currentField) { //for each field in this table
					...
					$currentFields[$fieldName] = $currentField;
					...
					if($indexType){ //see if indexType is set
						...
						$currentField['IndexPrefixLength'] = $indexesFinal[$fieldName]['IndexPrefixLength'];
						...
						$currentField['IndexPrefixLength'] = "";
					}
					...
					//track auto_increment columns as these are really picky in how they need adjusted with indexes
					if( stripos($currentField['Extra'], 'auto_increment') !== false ){
						$this->_autoIncrementFields[$tableName][$fieldName] = $currentField;
					}
				}
				$this->_lastSyncResults[$tableName]['deleted']=$fieldsDeleted;
				
				//1: drop keys and indexes, EXCLUDING auto_increment columns
				//NOTE: mysql will error if there is an auto_increment column without an index defined
				$this->DropAllKeys($tableName, false);
				
				//2: update fields that already exist. exclude auto_increment in this iteration as the auto_increment column *must* have a key which comes below. then we update the column to auto_increment after key is created
				//also, if primary key is being modified, it needs to remain "NOT NULL" until primary key definition is removed
				if(!empty($this->_fieldsToUpdate[$tableName])) {
					foreach($this->_fieldsToUpdate[$tableName] as $fieldName=>$newField){
						if($currentFields[$fieldName]["Key"] === 'PRI') $newField['Null'] = 'NO';  //All parts of a PRIMARY KEY must be NOT NULL. this is temporary so we can remove 'auto_increment', then we'll add null back later after primary key index is removed
						$this->UpdateField($tableName, $newField, false);
					}
				}
				
				//3: now drop remaining keys and indexes THAT ARE auto_increment columns
				//NOTE: mysql will error if there is an auto_increment column without an index defined
				$this->DropAllKeys($tableName, true);

				//4: then add indexes on fields that already exist
				if(!empty($this->_indexesToAdd[$tableName])){
					foreach($this->_indexesToAdd[$tableName] as $indexName=>$indexProperties){
						$indexType = $indexProperties['indexType'];
						$indexPrefixLength = $indexProperties['indexPrefixLength'] ?? null;
						$unique = ($indexType == "UNIQUE"); //indexType can be UNIQUE or NONUNIQUE
						$this->AddIndex($tableName, $indexName, $unique, $indexPrefixLength);
					}
				}

				//	add fulltext indexes on fields that already exist
				if(!empty($this->_fulltextToAdd[$tableName])){
					foreach($this->_fulltextToAdd[$tableName] as $fieldName=>$indexName){
						$this->AddFulltextIndex($tableName, $indexName);
					}
				}

				//	add new keys on fields that already exist
				if(!empty($this->_keysToAdd[$tableName])){
					foreach($this->_keysToAdd[$tableName] as $fieldName=>$keyName){
						$this->AddPrimaryKey($tableName, $keyName);
					}
				}
							
				//5: update fields that already exist to their final structure (includes auto-increment)
				$fieldsUpdated=0;
				if(!empty($this->_fieldsToUpdate[$tableName])){
					foreach($this->_fieldsToUpdate[$tableName] as $fieldName=>$newField){
				...
				//6: insert new fields (including keys and indexes on those fields)
	...
	//drop keys and indexes, EXCLUDING auto_increment columns
	//NOTE: mysql will error if there is an auto_increment column without an index defined
	private function DropAllKeys($tableName, $onlyAutoIncrementColumns){
		if( !empty($this->_keysToDrop[$tableName]) || !empty($this->_indexesToDrop[$tableName]) ){
			//drop indexes
			if(!empty($this->_indexesToDrop[$tableName])){
				foreach($this->_indexesToDrop[$tableName] as $fieldName=>$indexNamesArr){
					$isAutoIncrementColumn = !empty($this->_autoIncrementFields[$tableName][$fieldName]);
					if( ($onlyAutoIncrementColumns && $isAutoIncrementColumn) || (!$onlyAutoIncrementColumns && !$isAutoIncrementColumn) ){
						foreach($indexNamesArr as $indexName){
			...
			//drop keys
			if(!empty($this->_keysToDrop[$tableName])){
				foreach($this->_keysToDrop[$tableName] as $fieldName){
					$isAutoIncrementColumn = !empty($this->_autoIncrementFields[$tableName][$fieldName]);
					if( ($onlyAutoIncrementColumns && $isAutoIncrementColumn) || (!$onlyAutoIncrementColumns && !$isAutoIncrementColumn) ){
						$this->DropPrimaryKey($tableName); //should only be one
					}
				}
			}
		}
	...
	private function CheckForFieldUpdates($tableName, $currentField, $newField){
		...
		foreach ($currentField as $property=>$val) {
			switch ($property){
				...
				case "IndexType":
					if($currentField["IndexType"] != $newField["IndexType"] || $currentField['IndexPrefixLength'] != $newField["IndexPrefixLength"]){
						$this->Message(" ---- Field '<b>$fieldName</b>' :: Property '<b>$property</b>' difference. Current: '<b>"
							. ($currentField[$property] !== "" ? $currentField[$property] : "(not set)") . " - IndexPrefixLength: ".($currentField['IndexPrefixLength'] ? $currentField['IndexPrefixLength'] : "(not set)")
							. "</b>', New: '<b>"
							. ($newField[$property] !== "" ? $newField[$property] : "(not set)") . " - IndexPrefixLength: ".($newField['IndexPrefixLength'] ? $newField['IndexPrefixLength'] : "(not set)")
							. "</b>'");
						$updateField=true;

						$indexPrefixLength = $newField["IndexPrefixLength"];
						if($currentField["IndexType"] === $newField["IndexType"]){ //if index types are the same, only index prefix length changed
							if($newField["IndexType"]==="FULLTEXT"){
								//IndexPrefixLength not supported on fulltext indexes - https://dev.mysql.com/doc/refman/8.4/en/column-indexes.html
							}
							else {
								$dropIndex = true;
								$addIndex = true;
						...
						else{ //index types are different
						...
						if($currentField['Key'] == 'PRI'){
							$dropIndex = false; //we drop primary keys elsewhere
						}
						if($newField['Key'] == 'PRI'){
							$addIndex = false; //we drop primary keys elsewhere
						}
					}
					break;
				...
				default:
					if (strcmp($currentField[$property], $newField[$property])!=0) {
						//"Key" special case where MUL==UNI
						if($property=="Key" && $currentField[$property]=="MUL" && $newField[$property]=="UNI") break;
						
						//"Extra" has special case in MySQL v8+ for date/time columns with CURRENT_TIMESTAMP set. this DEFAULT_GENERATED flag gets added too. just skip it for now as it causes unnecessary syncs that do nothing
						if($property=="Extra" && $currentField[$property]=="DEFAULT_GENERATED") break;
		...
		if($updateField){ //regular update
			$this->_fieldsToUpdate[$tableName][$fieldName] = $newField;
		}
		if($dropIndex){ //remove unique or fulltext index
			$this->_indexesToDrop[$tableName][$fieldName][] = $currentField['IndexName'];
		}
		if($dropPrimaryKey){ //remove primary key
			$this->_keysToDrop[$tableName][$fieldName] = $fieldName;
		}
		...
		if($addFulltextIndex){ //add fulltext index
			$this->_fulltextToAdd[$tableName][$fieldName] = $fieldName;
		}
		if($addPrimaryKey){ //add primary key
			$this->_keysToAdd[$tableName][$fieldName] = $fieldName;
		}
	}
	...
	private function AddIndex($tableName, $fieldName, $unique, $prefixLength=null){
		if($this->_doInsert == false ) {
		...
		$this->Query("ALTER TABLE `$tableName` ADD $indexType (`$fieldName`".( $prefixLength ? "($prefixLength)" : "").")");
		return 1;
	}
	...
	private function UpdateField($tableName, $newDbField, $includeAutoIncrement=true){
		...
		$auto_increment = ($includeAutoIncrement ? $newDbField['Extra'] : '');
